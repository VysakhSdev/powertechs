    ...
    <section id={id} className="py-20 bg-slate-50 relative scroll-mt-20">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4">
          <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#0B2240]/10 text-[#0B2240] text-xs font-black uppercase tracking-wider border border-[#0B2240]/20">
            <Sparkles className="w-4 h-4 text-[#F2A900] fill-[#F2A900]" />
            <span>Brochure Capability Directory</span>
          </div>
          <h2 className="font-sans font-black text-3xl sm:text-4xl md:text-5xl text-[#0B2240] tracking-tight">
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            <div className="relative w-full md:max-w-md">
              ...
              <input 
                ...
                className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-sans focus:outline-none focus:ring-2 focus:ring-[#0B2240]/20 focus:border-[#0B2240] text-[#0B2240] transition-all"
              />
              {searchQuery && (
                <button 
                  onClick={() => setSearchQuery('')}
                  className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-semibold text-slate-400 hover:text-[#0B2240]"
                >
                  Clear
                </button>
              )}
            ...
            <div className="flex items-center gap-2 shrink-0">
              <SlidersHorizontal className="w-4 h-4 text-[#0B2240]/60" />
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-sans text-xs font-bold uppercase tracking-wider transition-all duration-300 cursor-pointer ${
                    isActive
                      ...
                      : 'bg-slate-50 text-[#0B2240] border border-slate-200 hover:bg-slate-100'
                  }`}
                >
                  {cat.icon}
                  <span>{cat.name}</span>
        ...
        {activeCategory !== 'all' && (
          <motion.div 
            ...
            className="grid grid-cols-1 md:grid-cols-12 gap-8 bg-[#0B2240] text-white p-8 rounded-3xl items-center shadow-lg border border-slate-800"
          >
          ...
          <AnimatePresence mode="popLayout">
            {filteredServices.map((service, index) => (
              <motion.div
                ...
                className="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-sm hover:shadow-xl hover:border-[#0B2240]/15 transition-all duration-300 flex flex-col justify-between group"
              >
                <div className="space-y-4">
                  {/* Category and Icon */}
                  <div className="flex items-center justify-between">
                    <div className="p-2.5 bg-slate-50 border border-slate-100 rounded-xl group-hover:bg-[#0B2240]/5 transition-colors">
                  ...
                  {/* Title */}
                  <h4 className="font-sans font-black text-sm text-[#0B2240] tracking-tight group-hover:text-[#F2A900] transition-colors leading-snug">
                    {service.title}
                  </h4>
                ...
                <div className="pt-6 mt-6 border-t border-slate-100 flex items-center justify-between">
                  <span className="font-mono text-[10px] font-bold text-[#0B2240]/40">
                    CODE PW-{service.id.toUpperCase()}
                  </span>
                  {onInquireService ? (
                    <button
                      onClick={() => onInquireService(service.title)}
                      className="inline-flex items-center gap-1 text-xs font-black text-[#0B2240] hover:text-[#F2A900] transition-colors uppercase tracking-widest cursor-pointer"
                    >
                      <span>Inquire</span>
                      <ArrowRight className="w-3.5 h-3.5" />
                    </button>
                  ) : (
                    <div className="flex items-center gap-1 text-xs font-black text-[#0B2240]/60 uppercase tracking-widest">
          ...
          {filteredServices.length === 0 && (
            <div className="col-span-full text-center py-12 bg-white rounded-3xl border border-slate-200 shadow-sm space-y-3">
              ...
              <button 
                ...
                className="text-xs font-extrabold text-[#0B2240] hover:text-[#F2A900] underline uppercase tracking-wider"
              >
                Reset Filters
              </button>
            </div>
